...
//	private static int tempFileCounter = 0;
	...
	private static FileInterface getFileInterface(java.net.URI uri) throws Exception { 
		...
		// HACK to fix JavaGAT confused idea of URIs.
		...
		org.gridlab.gat.URI tmpURI = new org.gridlab.gat.URI(uri.getScheme(), auth, separator + uri.getPath(), uri.getQuery(), uri.getRawFragment());
					
		ResourceDescription resource = findResourceDescription(uri);
	...
	public static void copy(java.net.URI from, java.net.URI to) throws Exception {
		...
		org.gridlab.gat.URI toURI = new org.gridlab.gat.URI(to.getScheme(), to.getAuthority(), 
				File.separator + to.getPath(), to.getQuery(), to.getRawFragment());
